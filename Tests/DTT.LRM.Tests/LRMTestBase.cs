...
namespace DTT.LRM.Tests
{
    public abstract class LRMTestBase : AbpIntegratedTestBase<LRMTestModule>
    {
        ...
        protected void UsingDbContext(Action<LRMDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<LRMDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<LRMDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<LRMDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<LRMDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LRMDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<LRMDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LRMDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<LRMDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<LRMDbContext, Task<T>> func)
        {
            T result;
